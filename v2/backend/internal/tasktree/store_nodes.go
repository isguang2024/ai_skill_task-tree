...
func (a *App) findNode(ctx context.Context, nodeID string, includeDeleted bool) (jsonMap, error) {
	rows, err := a.queryContext(ctx, `SELECT * FROM nodes WHERE id = ?`, nodeID)
	if err != nil {
		return nil, err
	}
...
func (a *App) listNodes(ctx context.Context, taskID string) ([]jsonMap, error) {
	...
	rows, err := a.queryContext(ctx, `SELECT * FROM nodes WHERE task_id = ? AND deleted_at IS NULL ORDER BY depth, sort_order, created_at`, taskID)
	if err != nil {
		return nil, err
	}
...
func (a *App) listNodesWithOptions(ctx context.Context, taskID string, opts nodeListOptions) (jsonMap, error) {
	...
	if strings.TrimSpace(opts.Query) != "" {
		...
		query += ` AND (n.title LIKE ? ESCAPE '\' OR n.path LIKE ? ESCAPE '\' OR n.instruction LIKE ? ESCAPE '\')`
...
func (a *App) createNode(ctx context.Context, taskID string, body nodeCreate) (jsonMap, error) {
	...
	err := a.withTx(ctx, func(txCtx context.Context) error {
		...
		if kind == "linked_task" {
			return &appError{Code: 400, Msg: "linked_task node type is disabled"}
		}
		role := normalizeNodeRole(kind, body.Role)
		if role == "stage" {
			kind = "group"
		}
		parentPath := ""
		...
		if parentID != "" {
			parent, err := a.findNode(txCtx, parentID, false)
			if err != nil {
				return err
			}
			if asString(parent["task_id"]) != taskID {
				return &appError{Code: 400, Msg: "parent node belongs to another task"}
			}
			if asString(parent["kind"]) == "linked_task" {
				return &appError{Code: 400, Msg: "cannot add child to a linked_task node"}
			}
			parentPath = asString(parent["path"])
			parentDepth = int(asFloat(parent["depth"]))
			parentRole = asString(parent["role"])
			parentStageNodeID = asString(parent["stage_node_id"])
			if asString(parent["kind"]) == "leaf" {
				...
				if _, err := a.execContext(txCtx, `UPDATE nodes
					SET kind = 'group',
					    role = CASE WHEN role = 'stage' THEN role ELSE 'container' END,
					    status = 'ready',
					    result = '',
					    progress = 0,
					    instruction = ?,
				...
				msg := "新增子节点，父节点转为分组节点"
				if err := a.insertEvent(txCtx, taskID, &parentID, "node_retyped", &msg, map[string]any{
					"from_kind": "leaf",
					"to_kind":   "group",
					"reason":    "child_added",
				}, nil, nil); err != nil {
		...
		if role == "stage" && parentID != "" {
			return &appError{Code: 400, Msg: "stage 只能创建在 root 级"}
		}
		depth := parentDepth + 1
		nodeKey := ""
		...
		if nodeKey == "" {
			var count int
			if parentID != "" {
				err = a.queryRowContext(txCtx, `SELECT COUNT(*) FROM nodes WHERE parent_node_id = ? AND deleted_at IS NULL`, parentID).Scan(&count)
			} else {
				err = a.queryRowContext(txCtx, `SELECT COUNT(*) FROM nodes WHERE task_id = ? AND parent_node_id IS NULL AND deleted_at IS NULL`, taskID).Scan(&count)
			}
			if err != nil {
				return err
			}
		...
		if body.StageNodeID != nil && strings.TrimSpace(*body.StageNodeID) != "" {
			stageNodeID = strings.TrimSpace(*body.StageNodeID)
		}
		if role == "stage" {
			stageNodeID = ""
		} else if stageNodeID != "" {
			stageNode, err := a.findNode(txCtx, stageNodeID, false)
			if err != nil {
				return err
			}
			if asString(stageNode["task_id"]) != taskID || asString(stageNode["role"]) != "stage" {
				return &appError{Code: 400, Msg: "stage_node_id 必须指向同任务内的 stage 节点"}
			}
		} else if parentRole == "stage" {
			stageNodeID = parentID
		} else if parentStageNodeID != "" {
			stageNodeID = parentStageNodeID
		}
		if parentWasLeaf && parentShouldCarry {
			...
			carryStageNodeID := parentStageNodeID
			if parentRole == "stage" {
				carryStageNodeID = parentID
			}
			if _, err := a.execContext(txCtx, `INSERT INTO nodes(id, task_id, parent_node_id, node_key, path, depth, kind, role, stage_node_id, title, instruction, acceptance_criteria_json, status, estimate, sort_order, metadata_json, created_by_type, created_by_id, creation_reason, created_at, updated_at)
				VALUES (?, ?, ?, ?, ?, ?, 'leaf', 'step', ?, ?, ?, ?, 'ready', ?, 0, ?, ?, ?, ?, ?, ?)`,
				carryID, taskID, parentID, "1", carryPath, parentDepth+1, nullable(carryStageNodeID), carryTitle, parentCarryInstruction, mustJSON(defaultCriteria(parentCarryAcceptance)), parentCarryEstimate, mustJSON(map[string]any{"generated_from_parent": true}), "system", nil, "split_from_parent", carryNow, carryNow); err != nil {
				return err
			}
			carryMsg := "父节点拆分后自动承接原任务内容"
			if err := a.insertEvent(txCtx, taskID, &carryID, "node_created", &carryMsg, map[string]any{
				"path":                carryPath,
				"kind":                "leaf",
		...
		if parentID != "" {
			var pathCount int
			if err := a.queryRowContext(txCtx, `SELECT COUNT(*) FROM nodes WHERE task_id = ? AND path = ? AND deleted_at IS NULL`, taskID, path).Scan(&pathCount); err != nil {
				return err
			}
			if pathCount > 0 {
				var count int
				if err := a.queryRowContext(txCtx, `SELECT COUNT(*) FROM nodes WHERE parent_node_id = ? AND deleted_at IS NULL`, parentID).Scan(&count); err != nil {
					return err
				}
				nodeKey = fmt.Sprintf("%d", count+1)
				path = parentPath + "/" + nodeKey
		...
		if role == "stage" && strings.TrimSpace(asString(task["current_stage_node_id"])) == "" {
			if _, err := a.execContext(txCtx, `UPDATE tasks SET current_stage_node_id = ?, updated_at = ?, version = version + 1 WHERE id = ?`, nodeID, utcNowISO(), taskID); err != nil {
				return err
			}
		}
...
func (a *App) retypeNodeToLeaf(ctx context.Context, nodeID string, body retypeBody) (jsonMap, error) {
	...
	err := a.withTx(ctx, func(txCtx context.Context) error {
		...
		case "leaf":
			...
			return nil
		case "linked_task":
			return &appError{Code: 400, Msg: "linked_task node cannot be converted to leaf"}
		case "group":
		default:
			return &appError{Code: 400, Msg: fmt.Sprintf("unsupported node kind %q", kind)}
		}
		var childCount int
		if err := a.queryRowContext(txCtx, `SELECT COUNT(*) FROM nodes WHERE parent_node_id = ? AND deleted_at IS NULL`, nodeID).Scan(&childCount); err != nil {
			return err
		}
		if childCount > 0 {
			return &appError{Code: 409, Msg: "group node still has child nodes"}
		}
		taskID := asString(node["task_id"])
		if _, err := a.execContext(txCtx, `UPDATE nodes
			SET kind = 'leaf',
			    role = CASE WHEN role = 'stage' THEN 'stage' ELSE 'step' END,
			    status = 'ready',
			    result = '',
			    progress = 0,
			    claimed_by_type = NULL,
		...
		if err := a.insertEvent(txCtx, taskID, &nodeID, "node_retyped", &msg, map[string]any{
			"from_kind": "group",
			"to_kind":   "leaf",
...
func (a *App) reportProgress(ctx context.Context, nodeID string, body progressBody) (jsonMap, error) {
	...
	err := a.withTx(ctx, func(txCtx context.Context) error {
		...
		case "group":
			return &appError{Code: 400, Msg: "cannot report progress on group node"}
		case "linked_task":
			return &appError{Code: 400, Msg: "cannot report progress on linked_task node"}
		}
		switch asString(node["status"]) {
		case "blocked":
			return &appError{Code: 409, Msg: "node is blocked, unblock first"}
		case "paused":
			return &appError{Code: 409, Msg: "node is paused, reopen first"}
		case "canceled", "done":
			return &appError{Code: 409, Msg: "node is closed, reopen first"}
		}
		...
		if body.IdempotencyKey != nil && *body.IdempotencyKey != "" {
			var count int
			if err := a.queryRowContext(txCtx, `SELECT COUNT(*) FROM events WHERE idempotency_key = ?`, *body.IdempotencyKey).Scan(&count); err != nil {
				return err
			}
			if count > 0 {
				updatedNode, err = a.findNode(txCtx, nodeID, false)
		...
		if err := a.insertEvent(txCtx, asString(node["task_id"]), &nodeID, "progress", body.Message, payload, body.Actor, body.IdempotencyKey); err != nil {
			return err
		}
		if runID != "" {
...
func (a *App) completeNode(ctx context.Context, nodeID string, body completeBody) (jsonMap, error) {
	...
	err := a.withTx(ctx, func(txCtx context.Context) error {
		...
		if asString(node["kind"]) != "leaf" {
			return &appError{Code: 400, Msg: "only leaf node can be completed directly"}
		}
		if body.IdempotencyKey != nil && *body.IdempotencyKey != "" {
			var count int
			if err := a.queryRowContext(txCtx, `SELECT COUNT(*) FROM events WHERE idempotency_key = ?`, *body.IdempotencyKey).Scan(&count); err != nil {
		...
		if err := a.insertEvent(txCtx, asString(node["task_id"]), &nodeID, "complete", body.Message, payload, body.Actor, body.IdempotencyKey); err != nil {
			return err
		}
		if _, err := a.addRunLog(txCtx, runID, runLogCreate{
			Kind:    "complete",
...
func (a *App) blockNode(ctx context.Context, nodeID string, body blockBody) (jsonMap, error) {
	...
	err := a.withTx(ctx, func(txCtx context.Context) error {
		...
		if asString(node["kind"]) != "leaf" {
			return &appError{Code: 400, Msg: "only leaf node can be blocked directly"}
		}
		if hasClosedResult(node) {
			return &appError{Code: 409, Msg: fmt.Sprintf("node status is %s", asString(node["status"]))}
		}
		if _, err := a.execContext(txCtx, `UPDATE nodes
			SET status = 'blocked',
			    result = '',
		...
		if err := a.insertEvent(txCtx, asString(node["task_id"]), &nodeID, "blocked", &body.Reason, nil, body.Actor, nil); err != nil {
			return err
		}
		if err := a.rollupTask(txCtx, asString(node["task_id"])); err != nil {
			return err
...
func (a *App) claimNode(ctx context.Context, nodeID string, body claimBody) (jsonMap, error) {
	...
	err := a.withTx(ctx, func(txCtx context.Context) error {
		...
		if asString(node["kind"]) == "group" {
			return &appError{Code: 400, Msg: "cannot claim group node"}
		}
		if hasClosedResult(node) {
			return &appError{Code: 409, Msg: fmt.Sprintf("node status is %s", asString(node["status"]))}
		}
		...
		if leaseActive(node) && asString(node["claimed_by_id"]) != strPtrValue(body.Actor.AgentID) {
			return &appError{Code: 409, Msg: fmt.Sprintf("already claimed by %s/%s until %s", asString(node["claimed_by_type"]), asString(node["claimed_by_id"]), asString(node["lease_until"]))}
		}
		leaseSeconds := 900
		...
		if _, err := a.execContext(txCtx, `UPDATE nodes SET claimed_by_type = ?, claimed_by_id = ?, lease_until = ?, status = CASE WHEN status IN ('pending','ready') THEN 'running' ELSE status END, result = '', updated_at = ?, version = version + 1 WHERE id = ?`,
			body.Actor.Tool, body.Actor.AgentID, leaseUntil, utcNowISO(), nodeID); err != nil {
			return err
		}
		if err := a.insertEvent(txCtx, asString(node["task_id"]), &nodeID, "claim", nil, map[string]any{"lease_until": leaseUntil}, &body.Actor, nil); err != nil {
			return err
		}
		if err := a.rollupTask(txCtx, asString(node["task_id"])); err != nil {
			return err
...
func (a *App) releaseNode(ctx context.Context, nodeID string) (jsonMap, error) {
	...
	err := a.withTx(ctx, func(txCtx context.Context) error {
		...
		if _, err := a.execContext(txCtx, `UPDATE nodes SET claimed_by_type = NULL, claimed_by_id = NULL, lease_until = NULL, status = ?, updated_at = ?, version = version + 1 WHERE id = ?`, status, utcNowISO(), nodeID); err != nil {
			return err
		}
		payload := map[string]any{"status": status}
		if err := a.insertEvent(txCtx, asString(node["task_id"]), &nodeID, "release", nil, payload, nil, nil); err != nil {
			return err
		}
		if err := a.rollupTask(txCtx, asString(node["task_id"])); err != nil {
			return err
...
func (a *App) sweepExpiredLeases(ctx context.Context) (int64, error) {
	var affected int64
	err := a.withTx(ctx, func(txCtx context.Context) error {
		rows, err := a.queryContext(txCtx, `SELECT DISTINCT task_id FROM nodes WHERE lease_until IS NOT NULL AND lease_until < ?`, utcNowISO())
		if err != nil {
			return err
		}
		...
		res, err := a.execContext(txCtx, `UPDATE nodes
...
func (a *App) insertEvent(ctx context.Context, taskID string, nodeID *string, kind string, message *string, payload map[string]any, act *actor, idem *string) error {
	if idem != nil && *idem != "" {
		var count int
		if err := a.queryRowContext(ctx, `SELECT COUNT(*) FROM events WHERE idempotency_key = ?`, *idem).Scan(&count); err != nil {
			return err
		}
		if count > 0 {
			return nil
	...
	_, err := a.execContext(ctx, `INSERT INTO events(id, task_id, node_id, type, message, payload_json, actor_type, actor_id, idempotency_key, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
