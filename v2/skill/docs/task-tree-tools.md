...
| 工具 | 说明 |
|------|------|
| `task_tree_list_tasks` | 列出任务（MCP 返回裁剪字段：id/title/status/summary_percent 等 12 字段，不含 wrapup_summary） |
| `task_tree_create_task` | 创建任务（可含初始 stages + 节点树，一步到位） |
| `task_tree_create_task(dry_run=true)` | 仅预演校验，不落库，返回 preview_tree 和依赖解析结果 |
| `task_tree_get_task` | 获取任务详情（全量字段） |
| `task_tree_update_task` | 更新任务 |
| `task_tree_delete_task` | 软删除任务（进回收站） |
| `task_tree_hard_delete_task` | 硬删除任务 |
| `task_tree_restore_task` | 从回收站恢复 |
| `task_tree_transition_task` | 任务状态流转（action: pause/reopen/cancel） |
| `task_tree_resume` | **核心**：获取任务完整上下文（task/tree/memory/events/runs/remaining/recommended_action） |
| `task_tree_next_node` | **核心**：获取下一个可执行节点（含 recommended_action + alternatives） |
...
| 工具 | 说明 |
|------|------|
| `task_tree_list_nodes` | 列出节点。**MCP 默认 `view_mode=summary`**（13 字段）。可选 slim/summary/detail/events。支持 kind/status 字符串或数组过滤 |
| `task_tree_create_node` | 创建节点（支持 depends_on / depends_on_keys） |
| `task_tree_batch_create_nodes` | **优化**：批量创建节点（N→1，事务原子） |
| `task_tree_get_node` | 获取节点详情（`include_context=true` 含完整上下文，等同 get_node_context） |
| `task_tree_update_node` | 更新节点（支持 depends_on / depends_on_keys） |
| `task_tree_get_node_context` | **核心**：节点完整上下文（ancestors/memory/runs/events/siblings/task/stage_summary） |
| `task_tree_focus_nodes` | 获取可执行的焦点节点 |
| `task_tree_move_node` | 移动节点 |
| `task_tree_reorder_nodes` | 重新排序同级节点 |
| `task_tree_retype_node` | 节点类型转换（group → leaf，需无子节点） |
...
| `task_tree_transition_node` | 节点状态流转（action: block/pause/reopen/unblock/cancel） |
...
| 工具 | 说明 |
|------|------|
| `task_tree_list_stages` | 列出阶段（MCP 返回裁剪字段 10 字段 + `{items:[...]}` 格式） |
...
| `task_tree_smart_search` | **核心**：全文检索（FTS5 + BM25），搜标题/指令/Memory/日志 |
...
| `task_tree_get_wrapup` | 获取任务收尾总结 |

## 其他

| 工具 | 说明 |
|------|------|
| `task_tree_work_items` | 获取当前可执行的工作项（MCP 返回裁剪字段 10 字段 + `{items:[...]}` 格式） |
| `task_tree_list_events` | 列出任务事件流（**MCP 默认 limit=20**，HTTP 默认 100） |
...
| `task_tree_search` | `smart_search`（内部已自动转发） |
| `task_tree_block_node` | `transition_node(action: 'block')` |

## MCP 参数使用说明

- **数组参数**（kind/status/type 等）：支持字符串 `"leaf"` 和数组 `["leaf"]` 两种写法，效果相同
- **MCP 输出**：自动移除 null 值和空 map 字段（omitEmpty），减少 token 消耗
- **view_mode 默认值**：MCP `list_nodes` 默认 `summary`；HTTP 无参数时返回全量字段
