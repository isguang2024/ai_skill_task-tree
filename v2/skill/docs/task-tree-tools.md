...
| `task_tree_create_node` | 创建节点（支持 depends_on） |
| `task_tree_batch_create_nodes` | **优化**：批量创建节点（N→1） |
| `task_tree_get_node` | 获取节点详情（`include_context=true` 含完整上下文） |
| `task_tree_update_node` | 更新节点 |
...
| `task_tree_complete` | **优化**：完成节点（自动 finish run + 内联 memory + 返回 next_node） |
