...
| 获取任务上下文 | resume | 含裁剪后的 task/memory + summary 树 + events + remaining + recommended_action |
...
| 列出节点 | list_nodes | **MCP 默认 summary**（13 字段）。可选 slim/summary/detail/events |
| 更新 Memory | patch_node_memory | 支持 append_execution_log 追加和 execution_log 覆盖两种模式 |
| 状态流转 | transition_node | action: block / pause / reopen / unblock / cancel |
| 收尾总结 | wrapup / get_wrapup | 写入/读取任务收尾总结 |
...
- **默认裁剪**：list_nodes 默认 summary、list_events 默认 limit=20、list_tasks/stages/work_items 返回精简字段
- **omitEmpty**：MCP 输出自动移除 null 值和空 map，减少 token 消耗
- **{items} 格式**：list_stages、work_items 的 MCP 返回统一为 `{items: [...]}` 格式
