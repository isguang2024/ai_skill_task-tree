...
func (a *App) getResumeContext(ctx context.Context, taskID, nodeID string, eventLimit int) (jsonMap, error) {
	...
	rows, err := a.db.QueryContext(ctx, `SELECT type, message, created_at, actor_type, actor_id FROM events WHERE node_id = ? ORDER BY created_at DESC LIMIT ?`, nodeID, eventLimit)
	if err != nil {
		return nil, err
	}
	...
	rows, err = a.db.QueryContext(ctx, `SELECT id, path, title, status, progress FROM nodes WHERE task_id = ? AND id != ? AND deleted_at IS NULL ORDER BY sort_order, created_at`, taskID, nodeID)
	if err != nil {
		return nil, err
	}
...
func (a *App) resumeTaskWithOptions(ctx context.Context, taskID string, nodeOpts nodeListOptions, eventOpts eventListOptions) (jsonMap, error) {
	...
	orderedLeafNodes := orderedExecutableLeaves(nodes)
	var nextNode jsonMap
	for _, node := range orderedLeafNodes {
		if asString(node["status"]) == "running" && !leaseActive(node) {
			nextNode = node
			break
		}
	}
	...
	if nodeOpts.ViewMode == "" {
		nodeOpts.ViewMode = "summary"
	}
	...
	resp := jsonMap{
		"task":          jsonMap{"task_id": task["id"], "task_key": task["task_key"], "title": task["title"], "goal": task["goal"], "status": task["status"], "summary_percent": task["summary_percent"]},
		"tree":          filteredNodesWrap["items"],
		"tree_has_more": filteredNodesWrap["has_more"],
		"tree_cursor":   filteredNodesWrap["next_cursor"],
		"remaining":     remaining,
...
func (a *App) search(ctx context.Context, q, kind string, limit int) (jsonMap, error) {
	...
	if kind == "" || kind == "all" || kind == "tasks" {
		rows, err := a.db.QueryContext(ctx, `SELECT * FROM tasks WHERE deleted_at IS NULL AND (title LIKE ? OR goal LIKE ?) ORDER BY updated_at DESC LIMIT ?`, like, like, limit)
		if err != nil {
			return nil, err
		}
	...
	if kind == "" || kind == "all" || kind == "nodes" {
		rows, err := a.db.QueryContext(ctx, `SELECT n.*, t.title AS task_title FROM nodes n JOIN tasks t ON t.id = n.task_id WHERE n.deleted_at IS NULL AND t.deleted_at IS NULL AND (n.title LIKE ? OR n.instruction LIKE ? OR n.path LIKE ?) ORDER BY n.updated_at DESC LIMIT ?`, like, like, like, limit)
		if err != nil {
			return nil, err
		}
		items, err := scanRows(rows)
...
func (a *App) listEventsScoped(ctx context.Context, taskID, nodeID string, includeDescendants bool, before, after string, limit int, opts eventListOptions) (jsonMap, error) {
	...
	if strings.TrimSpace(opts.Query) != "" {
		...
		query += ` AND (type LIKE ? OR message LIKE ?)`
...
func (a *App) descendantNodeIDs(ctx context.Context, taskID, nodeID string) ([]string, error) {
	...
	selectedPath := ""
	if taskID == "" {
		if err := a.db.QueryRowContext(ctx, `SELECT task_id, path FROM nodes WHERE id = ? AND deleted_at IS NULL`, nodeID).Scan(&taskID, &selectedPath); err != nil {
			return nil, &appError{Code: 404, Msg: fmt.Sprintf("node %s not found", nodeID)}
		}
	} else {
		if err := a.db.QueryRowContext(ctx, `SELECT path FROM nodes WHERE id = ? AND task_id = ? AND deleted_at IS NULL`, nodeID, taskID).Scan(&selectedPath); err != nil {
			return nil, &appError{Code: 404, Msg: fmt.Sprintf("node %s not found in task %s", nodeID, taskID)}
		}
	}
	rows, err := a.db.QueryContext(ctx, `SELECT id FROM nodes WHERE task_id = ? AND deleted_at IS NULL AND (path = ? OR path LIKE ?) ORDER BY path`, taskID, selectedPath, selectedPath+"/%")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (a *App) createArtifact(ctx context.Context, taskID string, body artifactCreate) (jsonMap, error) {
	...
	id := newID("art")
	if _, err := a.db.ExecContext(ctx, `INSERT INTO artifacts(id, task_id, node_id, kind, title, uri, meta_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		id, taskID, body.NodeID, kind, body.Title, body.URI, mustJSON(body.Meta), utcNowISO()); err != nil {
		return nil, err
	}
	msg := body.URI
	...
	if err := a.insertEvent(ctx, taskID, body.NodeID, "artifact", &msg, map[string]any{"artifact_id": id, "kind": kind, "uri": body.URI}, nil, nil); err != nil {
		return nil, err
	}
	rows, err := a.db.QueryContext(ctx, `SELECT * FROM artifacts WHERE id = ?`, id)
	if err != nil {
		return nil, err
	}
	items, err := scanRows(rows)
...
func (a *App) uploadArtifact(ctx context.Context, taskID string, r *http.Request) (jsonMap, error) {
	...
	return a.storeArtifactBytes(ctx, taskID, nodeID, kind, title, header.Filename, content, nil)
}
...
func (a *App) uploadArtifactBase64(ctx context.Context, taskID string, body artifactUpload) (jsonMap, error) {
	...
	return a.storeArtifactBytes(ctx, taskID, body.NodeID, kind, title, body.Filename, content, body.Meta)
}
...
func (a *App) storeArtifactBytes(ctx context.Context, taskID string, nodeID *string, kind, title, filename string, content []byte, extraMeta map[string]any) (jsonMap, error) {
	if _, err := a.getTask(ctx, taskID, false); err != nil {
		return nil, err
	}
	if nodeID != nil && *nodeID != "" {
...
func (a *App) getArtifactFile(ctx context.Context, artifactID string) ([]byte, string, error) {
	rows, err := a.db.QueryContext(ctx, `SELECT * FROM artifacts WHERE id = ?`, artifactID)
	if err != nil {
		return nil, "", err
	}
	items, err := scanRows(rows)
