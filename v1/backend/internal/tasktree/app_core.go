...
func (a *App) migrate() error {
	...
	var current int
	if err := a.db.QueryRow(`SELECT COALESCE(MAX(version), 0) FROM schema_version`).Scan(&current); err != nil {
		return err
	}
	...
	for _, name := range names {
		...
		content, err := os.ReadFile(filepath.Join(resolveUpwardPath("migrations"), name))
		if err != nil {
			return err
		}
		if _, err := a.db.Exec(string(content)); err != nil && !strings.Contains(err.Error(), "duplicate column name") {
			return err
		}
		if _, err := a.db.Exec(`INSERT OR REPLACE INTO schema_version(version, applied_at) VALUES (?, ?)`, version, utcNowISO()); err != nil {
			return err
		}
	}
	return nil
